...
(shape assertions, not product behavior), and (E5) the independent native
Windows acceptance record
([acceptance/windows-native.md](acceptance/windows-native.md)).

## Loading paths (where each asset lands)

| Target | Skills | Global rules | MCP (stdio) |
| --- | --- | --- | --- |
| Codex | `~/.agents/skills/<name>/` (E1/E5: shared with DSH, auto-discovered) | `~/.codex/AGENTS.md`, marker-delimited blocks (E2: same mechanism as the author's `manage-global-agents.py`) | `~/.codex/config.toml` `[mcp_servers.<name>]` command/args/env (E2/E3: Codex config format) |
...
| DSH | `~/.agents/skills/<name>/` (E1/E5: native auto-load, verified) | `$DSH_HOME/AGENTS.md`, marker blocks (E2: `manage-global-agents.py` dsh target) | `$DSH_HOME/profiles/<profile>/cordis.patch.yml`, `dsh-mcp-client` insert entries (E2: `apply-dsh-mcp.py`; E1/E5: verified live profile) |
...
Evidence: E1/E5 (Codex/DSH), E3 (Claude personal skills directory and
...
Evidence: E2/E1/E5 for Codex and DSH formats; E3 for Claude's `.claude.json`
...
- **Native evidence on Windows** — the independent Windows acceptance record
  passes the v1 lifecycle and DSH process checks, with explicit skips and
  limitations documented there. CI green on `windows-latest` remains
  automated shape evidence, not a substitute for that native record.
